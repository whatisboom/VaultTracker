...
## Build & release
- **Tag-driven, via GitHub Actions.** `.github/workflows/release.yml` triggers on `vX.Y.Z` tag push: runs `BigWigsMods/packager@v2` (reads `.pkgmeta`, builds, publishes to CurseForge via the `CF_API_KEY` secret, creates the GitHub release), then posts a Discord embed (via `DISCORD_WEBHOOK_URL`) built from that tag's `CHANGELOG.md` section. CurseForge's own GitHub App integration is disabled — Actions is the sole trigger. Untagged pushes do nothing; tag name sets the channel (no "alpha"/"beta" ⇒ release).
